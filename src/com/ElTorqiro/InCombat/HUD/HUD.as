...
import com.ElTorqiro.InCombat.HUD.SettingsPacks;

/**
 * 
 * 
 */
class com.ElTorqiro.InCombat.HUD.HUD extends UIComponent {
	...
	private var _checkCombatStateTimerID:Number;
	private var _findDefaultCombatIndicatorThrashCount:Number = 0;
	private var _bars:Object;
	
	// internal movieclips
	private var m_Combat:MovieClip;
	private var m_Threatened:MovieClip;
	private var m_Overlay:MovieClip;

	// internal states
	private var _mouseDown:Number = -1;
	private var _lastPolledCombatState:String;
	...
	private var _inCombatHUDState:DistributedValue;
	...
	public function HUD() {
		...
		_inCombatHUDState = DistributedValue.Create( AddonInfo.ID + "_HUD_State" );
	...
	private function configUI() : Void {
		...
		_bars = { combat: m_Combat, threatened: m_Threatened };
		...
		// start listening for combat status events
		_character.SignalToggleCombat.Connect( CheckCombatState, this );
	...
	public function onUnload():Void {
		...
		_character.SignalToggleCombat.Disconnect( CheckCombatState, this );
		_configMode.SignalChanged.Connect( stateTriggerHandler, this );
	...
	public function LoadIcons() : Void {
		...
		if ( useCustomIcons ) {
			...
			m_Combat.attachMovie( "com.ElTorqiro.InCombat.HUD.Icon.Default", "m_Icon", m_Combat.m_Icon.getDepth() );
			m_Threatened.attachMovie( "com.ElTorqiro.InCombat.HUD.Icon.Default", "m_Icon", m_Threatened.m_Icon.getDepth() );
			Layout();
		}
	}
	...
	private function onLoadError(target:MovieClip) : Void {
		target._parent.attachMovie( "com.ElTorqiro.InCombat.HUD.Icon.Error", "m_Icon", target.getDepth() );
		Layout();
	}
	...
	private function CheckCombatState() : Void {

		clearTimeout( _checkCombatStateTimerID );
		_checkCombatStateTimerID = undefined;
		
		// if changed to combat, only way to get out of it is to go into not combat, so poll doesn't need to be restarted
		if ( combatState == "combat" ) {
			_lastPolledCombatState = "combat";
			stateTriggerHandler();
		}
		
		// if threatened, could end up going into combat, so resume the poll
		else if ( combatState == "threatened" ) {
			if( _lastPolledCombatState != "threatened" ) {
				stateTriggerHandler();
			}
			
			_lastPolledCombatState = "threatened";
			_checkCombatStateTimerID = setTimeout( Delegate.create(this, CheckCombatState), 500 );
		}
		
		// otherwise poll doesn't need restarting
		else {
			_lastPolledCombatState = "none";
			stateTriggerHandler();
		}
	}
	...
	private function draw() : Void {
		...
		// overlay wrapper
		var combatRect:Object = m_Combat.getBounds(m_Combat);
		var threatenedRect:Object = m_Threatened.getBounds(m_Threatened);
		
		m_Overlay._x = Math.min(combatRect.xMin, threatenedRect.xMin);
		m_Overlay._y = Math.min(combatRect.yMin, threatenedRect.yMin);
		m_Overlay._width = Math.max( Math.abs(combatRect.xMin) + Math.abs(combatRect.xMax), Math.abs(threatenedRect.xMin) + Math.abs(threatenedRect.xMax) );
		m_Overlay._height = Math.max( Math.abs(combatRect.yMin) + Math.abs(combatRect.yMax), Math.abs(threatenedRect.yMin) + Math.abs(threatenedRect.yMax) );
	...
	public function set hudEnabled(value:Boolean):Void {
		...
		_inCombatHUDState.SetValue( _hudEnabled ? "enabled" : "disabled" );
